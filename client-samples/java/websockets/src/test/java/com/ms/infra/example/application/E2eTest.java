...
public class E2eTest {
    ...
    void e2eTest() throws Exception {
        ...
        MicroprofileConfigService microprofileConfigServiceSpy = Mockito.spy(new MicroprofileConfigService());
        Mockito.doReturn(Mockito.mock(PrivateKey.class)).when(microprofileConfigServiceSpy).getPrivateKey();
        Mockito.doReturn(Mockito.mock(X509Certificate.class)).when(microprofileConfigServiceSpy).getPublicCertificate();
        ...
        Mockito.doReturn(MOCK_BEARER_TOKEN).when(webSocketServiceSpy).getAuthToken();
